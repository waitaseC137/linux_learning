...
(function () {
  ...
  const T = EN ? {
    loading: 'loading', copy: 'copy', prev: '← previous', next: 'next →',
    loadErr: function (m) { return 'Could not load file (' + m + ').'; },
    ghOpen: 'Open on GitHub ↗'
  } : {
    loading: 'yükleniyor', copy: 'kopyala', prev: '← önceki', next: 'sonraki →',
    loadErr: function (m) { return 'Dosya yüklenemedi (' + m + ').'; },
    ghOpen: "GitHub'da aç ↗"
  };
  ...
  async function loadDoc(rel) {
    ...
    elContent.innerHTML = '<div class="reader"><div class="state">' + T.loading + '<span class="blink">_</span></div></div>';
    window.scrollTo(0, 0);
    ...
    try {
      ...
      elContent.innerHTML =
        ...
        T.loadErr(e.message) + '<br><br>' +
        '<a class="gh-view" href="' + REPO_BLOB + encodeURI(BASE + rel) + '" target="_blank" rel="noopener">' + T.ghOpen + '</a>' +
        '</div></div>';
      return;
    }
    ...
    const navHtml =
      '<div class="doc-nav">' +
        (prev ? '<a class="prev" href="#' + encodeURI(prev.f) + '"><div class="dir">' + T.prev + '</div><div class="dt">' + prev.t + '</div></a>' : '<a class="empty"></a>') +
        (next ? '<a class="next" href="#' + encodeURI(next.f) + '"><div class="dir">' + T.next + '</div><div class="dt">' + next.t + '</div></a>' : '<a class="empty"></a>') +
      '</div>';
