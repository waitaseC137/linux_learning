...
```bash
# Ciphertext: SQJUBNGCD... → English: EATSORNIHC...
...
### Stream Cipher and XOR

**Stream Cipher:** XORs each byte individually with a "pseudo-random" **keystream**.

```
cipher_byte     = plaintext_byte  XOR keystream_byte
plaintext_byte  = cipher_byte     XOR keystream_byte
```

**The fundamental property of XOR:**
```
A XOR B = C
C XOR B = A   ← if you XOR again with the same value, you return to the original
```

**Vulnerability — Repeating Keystream:**

In implementations that use a weak random number generator (RNG), the keystream repeats at a certain length. In this case:

1. Encrypt the binary with known plaintext (`AAAA...`)
2. For each position: `keystream[i] = ord(known_cipher[i]) - ord('A')`
3. The target ciphertext: `plaintext[i] = (ord(cipher[i]) - keystream[i]) % 26 + ord('A')`
...
> 💡 Secure stream ciphers (AES-CTR, ChaCha20) use a cryptographically secure RNG — the keystream never repeats.
